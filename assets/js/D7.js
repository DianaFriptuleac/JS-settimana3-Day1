...
function stringConc(stringa1, stringa2){
  let newString;
  newString = stringa1.slice(0, 2).concat(stringa2.slice(-3));
console.log(newString.toUpperCase());
}
stringConc('Milano', 'Venezia');
...
function random(){
  for (let i = 0; i < 10; i ++){
    ...
    if (!elemento.includes(numberRandom)){
      elemento.push(numberRandom);
    }else{
      i--;
    }
  }
...
function randomFunction(){
  const numArray = [];
  for (let i = 0; i < 10; i++){
    let newNumb = Math.floor(Math.random()*100)+1;
    if (numArray.indexOf(newNumb) === -1){
      numArray.push(newNumb);
    }else{
      i--;
    }
...
const array1=['11','84','18','52','98','15','25','100','51'];
function even(){
const numeroFiltrato = array1.filter(index => index % 2 ===0);
console.log (numeroFiltrato);
}
even ();
/* ESERCIZIO 4 (forEach)
...
const array3=[2, 1, 5 ,10];
const totaleArray = array3.reduce((sum, item) => sum + item,0);
console.log(totaleArray);

/*console.log('*****esercizio5*****');

function sum2(){
  let mySum2 = array3.reduce((total, item) => total + item, 0);
  cconsole.log(mySum2)
}
sum2();*/
...
*/
console.log('*****esercizio6 soluzione1*****');

const array4 = [2, 1, 5 ,10];
const number = 3;
const totalNumbers = array4.map((item) => {
  return item += number;
});
...
function incremento(numbAr, value){
  const arrayIncremento = numbAr.map(item =>{
    return item + value;
  });
  console.log(arrayIncremento);
}
incremento([2,1,5,10],3);
...
/* ESERCIZIO 11 (map)Scrivi una funzione per creare un array con solamente i titoli dei film contenuti nell'array fornito.*/
...
/* ESERCIZIO 12 (filter)
  ...
  */
  ...
  function newMov(){
  const newMovies = movies.filter((item )=> { return item.Year > '2000'; 
    });
